...
function CaptureArtillery()
		...
		for n = 1, #forcesList do
			...
			for k,v in pairs(entities.Units) do
				...
				if v.IsArtillery then	
					...
					UpdateMACHLuaLog("\t\t Found victorious artillery unit distance from captured artillery unit: "..distance)
	...
	local function compare_artilleryForcesTable()
		...
		for j = 1, #GLOBAL_artilleryForcesTable do	
			...
			--UpdateMACHLuaLog("HJERE:"..tostring(GLOBAL_artilleryForcesTable[j][1]).." - "..tostring(GLOBAL_artilleryForcesTable[j][2]).." - "..tostring(GLOBAL_artilleryForcesTable[j][3]).." - "..tostring(GLOBAL_artilleryForcesTable[j][4]))	
			local found = false
			...
			--UpdateMACHLuaLog("HERE "..tostring(GLOBAL_artilleryForcesTable[j][11]).."-"..tostring(GLOBAL_artilleryForcesTable[j][1]).."-"..tostring(GLOBAL_artilleryForcesTable[j][2]).."-"..tostring(GLOBAL_artilleryForcesTable[j][4]).."-"..tostring(GLOBAL_artilleryForcesTable[j][8]))
			...
			if (found==false)  then
				...
				numMenLost = GLOBAL_artilleryForcesTable[j][6]
					...
					--UpdateMACHLuaLog("NOT FOUND: "..tostring(GLOBAL_artilleryForcesTable[j][1]).." - "..tostring(GLOBAL_artilleryForcesTable[j][2]).." - "..tostring(GLOBAL_artilleryForcesTable[j][3]).." - "..tostring(GLOBAL_artilleryForcesTable[j][4]))
				...
				if tostring(victor_Faction_Key) == tostring(GLOBAL_artilleryForcesTable[j][1]) then
					--UpdateMACHLuaLog("Unit is part of victor faction: "..tostring(GLOBAL_artilleryForcesTable[j][1]))
				elseif (is_loser_faction(tostring(GLOBAL_artilleryForcesTable[j][1]))) then
					UpdateMACHLuaLog("Artillery unit is part of loser faction and doesn't exist any more: "..tostring(GLOBAL_artilleryForcesTable[j][1]))
					
					--UpdateMACHLuaLog("ARGUMENTS PASSED IN: "..GLOBAL_artilleryForcesTable[j][2]..GLOBAL_artilleryForcesTable[j][4])
			...
			elseif (found == true) and (lossGuns == true) then						
				...
				elseif (is_loser_faction(tostring(GLOBAL_artilleryForcesTable[j][1]))) then
					...
					--UpdateMACHLuaLog("Unit is part of loser faction AND LOST GUNS: "..tostring(GLOBAL_artilleryForcesTable[j][1]))
				end				
			end
		end
			...
			for i = 1, #forcesList do
				...
				for k,v in pairs(entities.Units) do
					if v.IsArtillery then
						...
						GLOBAL_artilleryForcesTable[a][1] = tostring(v2.Key) -- faction key
	...
	--character completed battle event to determine victor and sometimes loser faction
	events.CharacterCompletedBattle[#events.CharacterCompletedBattle+1] = function(context)
		...
		for k, v in pairs(faction_list) do
			...
			if conditions.CharacterFactionName(v.Key, context) then
				--UpdateMACHLuaLog("Character completed battle and belongs to faction: "..tostring(v.Key))
				...
				if conditions.CharacterWonBattle(context) then
					...
					--UpdateMACHLuaLog("Victor faction: "..tostring(v.Key)) 
				else
					loser_Faction_Key = v.Key
					--UpdateMACHLuaLog("Loser faction: "..tostring(v.Key)) 
				end
				
				if conditions.CharacterWasAttacker(context) then
					--UpdateMACHLuaLog("Character attacker faction name: "..tostring(v.Key)) 
					attacker_Faction_Key = v.Key
					attacker_character_context = context
				else 
					--UpdateMACHLuaLog("Character defender faction name: "..tostring(v.Key)) 
	...
	--faction turn start event to refresh GLOBAL_artilleryForcesTable table
	events.FactionTurnStart[#events.FactionTurnStart+1] = function(context)
...
function InterruptableSupplyLines()
						...
						for count = 1, #GLOBAL_characterNames_list do
							...
							if  not (string.find(long_supply_name, GLOBAL_characterNames_list[count][4], 1) == nil) then														
								if conditions.CharacterForename(GLOBAL_characterNames_list[count][2], context) then	
									found = true
									local points = 0
									--UpdateMACHLuaLog(longSupplyTable[j][3])
									...
									--UpdateMACHLuaLog(longSupplyTable[j][4].."'s fleet "..bad_count)
									
									for h = 1, #GLOBAL_SettlementNames_list do
										--UpdateMACHLuaLog("hi "..GLOBAL_SettlementNames_list[h][2].." : "..bad_count)
										--UpdateMACHLuaLog(string.find(GLOBAL_SettlementNames_list[h][2], "start_pos_settlements_onscreen_name_"..regionToSettlementList[longSupplyTable[j][4]], 1))
										--UpdateMACHLuaLog(regionToSettlementList["bosnia"])
										--UpdateMACHLuaLog("start_pos_settlements_onscreen_name_"..regionToSettlementList[longSupplyTable[j][4]])
	...
	function determineInSettlement(faction, army)
		...
		for k = 1, #regions do
			...
			--UpdateMACHLuaLog(faction.." - "..region_key.." distance: "..distance)
								...
								if #blockingSupplyForces[k] > 0 then
									for p = 1, #blockingSupplyForces[k] do
										if blockingSupplyForces[k][p][1] == forcesList[i].Name and not blockingSupplyForces[k][p][2] == CampaignUI.CurrentTurn() then									
											--UpdateMACHLuaLog("eeerereef "..k.."-"..faction.." - "..forcesList[i].Name)
	...
	function determineClosestCapital(faction_key)
		UpdateMACHLuaLog("InterruptableSupplyLines.determineClosestCapital")
		...
		for i = 1, #forcesList do
			...
			if nearestRegionCapital ~= nil then
				...
				if nearest > WALI.MACH_LongSupplyLines_1 and faction_key == current_faction_turn and not (determineInSettlement(faction_key, forcesList[i])) then
					local found = false
					local ind = nil
		...
		if faction_key ~= current_faction_turn then
			...
			for i = 1, #forcesList do
				...
				--determine if long supply line
				if nearest > WALI.MACH_LongSupplyLines_1 and not (determineInSettlement(faction_key, forcesList[i])) then
...
function SurpriseAttack()
	...
	events.FactionTurnStart[#events.FactionTurnStart+1] = function(context)
		...
		for k, v in pairs(faction_list) do
			if conditions.FactionName(v.Key, context) then
				faction_turn = v.Key
				UpdateMACHLuaLog("Current faction turn: "..tostring(faction_turn))
			end
		end
...
function AI_Attrition()
	...
	events.FactionTurnStart[#events.FactionTurnStart+1] = function(context)
		...
		if conditions.TurnNumber(context) >= 0 then
			...
			--UpdateMACHLuaLog("faction key namne: "..GLOBAL_current_faction_key_name)
